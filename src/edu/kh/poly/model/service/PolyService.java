...
public class PolyService {
	...
	public void ex1() {
		...
		car3.setFuel("휘발유");
		car3.setWheel(4);
		//car3.setdiscountOffer(double)0.5);
		...
		Car[] arr = new Car[3]; // 부모타입 참조변수 배연 선언 및 할당
								// 각 배열요소가 Car타입 참조 변수여야 함.
		
		arr[0] = car; // Car주소 = Car객체 (Car 주소는 Car타입 객체)
		//arr[0] Car 참조변수
		
		arr[1] = car2; // Tesla 주소 == Tesla객체
		// car2는 tesla객체지만 업캐스팅 되어서 문제 x
		//arr[1] Car 참조변수
	...
	public void ex2() {
		...
		// == arr[1] = new Tesla();
		// == arr[2] = new Spark(); 랑 똑같은 모양
		
		// arr[0]; // Car
		// arr[1]; // Tesla
		// arr[2]; // Spark // 컴퓨터는 이렇게 값 들어간걸 모름
		
		// 컴퓨터가 알 수있게 새로운 연산자 사용
	...
	public void ex3() {
		...
		// 먼저 c1을 Telsa로 형변환 해주고, getBatteryCapacity호출해야함.
		
		// <효율적인 다운 캐스팅 방법>
		// - 얕은 복사 (주소 복사)를 이용한다.
		Tesla t1 = (Tesla)c1;
					// (Tesla)를 뺴면 c1은 Car타입
		// 자식참조변수로 부모객체 창조? ===== Tesla t1 = c1; / (반대는 업캐스팅으로 가능)
		// c1이 참조하는게 Tesla라는걸 알기때문에 Tesla로 강제 형변환 가능
		// Tesla = Tesla 이런 모양이 되고 같은 자료형 연산은 가능하니 같은 주소를 복사시켜버림.(얕은복사)
		
		//c1이 원래 Car타입 참조변수인데 먼저 Tesla로 강제형변환을 해주고 Tesla 참조변수에 넣어준거임
		// 결과적으로 t1을 활용하게됨
	...
	// !!! 다운캐스팅 주의사항 !!!
	public void ex4() {
		// 다운캐스팅 중 오류사항이 있을 수있음, 상황을 만들어보자
		
		Car c1 = new Tesla(); // 테슬라 객체를 부모타입 참조변수에 넣어놨음 // 밑에 식 실패
		//Car c1 = new Spark(); // 성공
		
		
		//Spark s1 = (Spark)c1; // 다운캐스팅, 이미 위에서 자식형태로 만들었는데 또 자식으로 강제 형변환
		...
		// -> 다운캐스팅 주의사항 : 강제 형변환을 잘못하면 안됨
		
		// instanceof와 같이 사용
		if(c1 instanceof Spark) {
			Spark s1 = (Spark)c1; // 자료형이 맞을 때 다운 캐스팅
	...
	public void printCar(Car temp) { // 자료형이 Car가 아니면 세개 다 출력x
	...
	public Car createCar(int num) {
		...
		// 반환형이 Car이지만
		// case 2,3이면 Car의 자식 객체의 주소가 반환된다.
	}
	...
	public void ex5() {
		...
		Car c1 = new Car("경유 엔진", "경우", 8);
		System.out.println(c1.getEngine());
		// Car 객체에 있는 get Engine() 메소드를 호출 == 바인딩
